...
if Telescope == "CTAN_alpha": #compute the values we need if the telescope selected is CTAN_alpha (needs to be changed as the Response function has to be changed)
    ...
    E_final = Etrue
    Eest_final = Eest[9:24]
    
    tau = log_interp1d(E_EBL, tau_EBL, Etrue) #interpolate the tau values to have the same bins as the migration matrix and the data.
...
if Forward_folding:
    ...
    mu_off = bckgmu_final 
    my_generator2 = np.random.default_rng(0)
